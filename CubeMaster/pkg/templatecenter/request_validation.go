...
func normalizeTemplateExposedPorts(ports []int32) ([]int32, error) {
	...
	// No count limit on template-level exposed ports. The ports baked into a
	// template are chosen by the operator (image author), not by sandbox
	// users, so a numeric cap only ever broke legitimate runtime images (the
	// dc-danus template alone needs envd, code interpreter, Rust sandbox
	// server REST + Cap'n Proto and code-server). Per-port validation and
	// de-duplication above still apply. Cubelet accepts any count too
	// (see Cubelet/services/cubebox/check.go).
	return normalized, nil
}

// countCustomTemplateExposedPorts reports how many of the given ports are not
// cubelet defaults (envd 49983). Informational only - it no longer gates the
// request.
